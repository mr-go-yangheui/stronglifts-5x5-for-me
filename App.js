...
const EXERCISE_NAMES={squat:"스쿼트",bench:"벤치프레스",row:"바벨 로우",ohp:"오버헤드프레스",deadlift:"데드리프트"};

function loadState(){
  const saved=localStorage.getItem("sl5x5_state");
  if(saved)return JSON.parse(saved);
  return{sessionCount:0,weights:{...DEFAULT_WEIGHTS},failCount:{squat:0,bench:0,row:0,ohp:0,deadlift:0},history:[]};
}
